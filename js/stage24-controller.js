(function(K){
  ...
  var oldItemAction=actions.itemAction;actions.itemAction=function(action){var state=K.State.data,item=selectedItem,kind=projectileKind(action,item),prepaid=false;if(busy)return false;if(action==='shoot'&&sleeping(state))return oldItemAction.apply(this,arguments);if(!kind)return oldItemAction.apply(this,arguments);if(action==='shoot')prepaid=prepayArrow(state,item);return begin(state,kind,item,function(){if(prepaid)item.quantity++;oldItemAction.call(actions,action);});};
  var oldShoot=actions.shootArrow;actions.shootArrow=function(){var state=K.State.data,item=state.player.equipment.arrow,prepaid=false;if(busy)return false;if(sleeping(state))return oldShoot.apply(this,arguments);if(!item||item.quantity<=0)return oldShoot.apply(this,arguments);prepaid=prepayArrow(state,item);return begin(state,'arrow',item,function(){if(prepaid)item.quantity++;oldShoot.call(actions);});};
