(function(K){
  ...
  function setLevel(player,target){target=Math.max(1,Math.min(MAX_LEVEL,Math.floor(Number(target)||1)));var simulated={level:1,exp:target===1?0:EXP_TABLE[target-1],maxHp:24,hp:24};applyLevels(simulated);player.level=simulated.level;player.exp=simulated.exp;player.maxHp=simulated.maxHp;player.hp=simulated.hp;return player;}
  K.Progression={EXP_TABLE:EXP_TABLE,MAX_LEVEL:MAX_LEVEL,nextThreshold:threshold,remaining:function(player){var next=threshold(player.level);return next===null?null:Math.max(0,next-player.exp);},addExp:function(player,amount){player.exp=Math.max(0,(player.exp||0)+Math.max(0,amount||0));return applyLevels(player);},applyLevels:applyLevels,setLevel:setLevel,grantNextLevel:function(player){var next=threshold(player.level);if(next===null)return[];player.exp=Math.max(player.exp||0,next);return applyLevels(player);}};
