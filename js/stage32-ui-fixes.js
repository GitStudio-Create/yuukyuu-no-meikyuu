(function(K){
  ...
  function drawMap(ctx,canvas,data,mode){
    ...
    data.tiles.forEach(function(t){ctx.fillStyle=t.floor?(t.visible?(mode==='standalone'?'#52685f':'rgba(118,174,202,.46)'):(mode==='standalone'?'#26342f':'rgba(76,110,132,.32)')):(t.visible?(mode==='standalone'?'#323a3a':'rgba(74,86,92,.38)'):(mode==='standalone'?'#141918':'rgba(42,52,58,.26)'));ctx.fillRect(Math.round(ox+t.x*s),Math.round(oy+t.y*s),Math.ceil(s),Math.ceil(s));});
